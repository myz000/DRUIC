...
class AttLayer(layers.Layer):
    ...
    def __init__(self, dim=200, r=0.5, seed=0, **kwargs):
        """Initialization steps for AttLayer.
    ...
    def build(self, input_shape):
        """Initialization for variables in AttLayer
        There are there variables in AttLayer, i.e. W, b and q.
    ...
    def call(self, inputs, mask=None, **kwargs):
        ...
        """
        attention_weight = self.get_attention_weight(inputs, mask)
        weighted_input = inputs * attention_weight
        return K.sum(weighted_input, axis=1)
    
    def get_attention_weight(self, inputs, mask=None, **kwargs):
        #attention = K.tanh(K.dot(inputs, self.W) + self.b)
        attention = K.dot(inputs, self.W) + self.b
        attention = self.r * K.tanh(K.dot(attention, self.q))
